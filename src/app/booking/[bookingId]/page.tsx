...
export default async function BookingPage({
  ...
  if (!res.ok) {
    return (
      <div className="max-w-3xl mx-auto p-6 mt-24 text-center bg-white rounded-xl shadow">
        <h1 className="text-xl font-bold text-red-600">
          Booking not found or Bkash sandbox unavailable.
        </h1>
      </div>
    );
  }
  ...
  if (!profileRes.ok) return <div className="p-8">Turf Profile not found</div>;

  const profileJson = await profileRes.json();
  const turfProfile = profileJson.data;
